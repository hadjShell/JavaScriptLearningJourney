...
class Workout {
  ...
  constructor(data) {
    ...
    this.#date = Intl.DateTimeFormat("en-GB", {
      month: "long",
      day: "numeric",
    }).format(new Date());
    this.#name = data.name;
...
class App {
  ...
  _renderMap(position) {
    ...
    L.popup({ closeOnClick: false, autoClose: false })
      .setLatLng(this.#currentCoords)
      .setContent("Your current position")
      .openOn(this.#map);
    // click map
    this.#map.on("click", e => {
    ...
    workoutElement.innerHTML = `<h2 class="workout__title">${title}</h2>
        ...
        <div class="workout__details">
          ...
          <span class="workout__value">${
            name === "running" ? workout.getPace() : workout.getSpeed()
          }</span>
        ...
        <div class="workout__details">
          ...
          <span class="workout__value">${
            name === "running" ? workout.getCadence() : workout.getElevation()
          }</span>
  ...
  _addSubmissionListener() {
    ...
    form.addEventListener("submit", e => {
      ...
      else submission.elevation = Number(inputElevation.value);
      inputDistance.value =
        inputDuration.value =
        inputCadence.value =
  ...
  static searchCoordsById(id) {
    return App.workouts
      .find(workout => workout.getId() === Number(id))
      .getCoords();
  }
