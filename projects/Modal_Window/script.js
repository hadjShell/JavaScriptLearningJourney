...
containerNavLinks.addEventListener("mouseover", function (e) {
    ...
    [...this.children].forEach(e => { if (e !== hovered) e.style.opacity = "0.5"; });
...
containerTab.addEventListener("click", function (e) {
    ...
    // Active tab
    [...this.children].forEach(btn => btn.classList.remove("operations__tab--active"));
    clicked.classList.add("operations__tab--active");
    // Active content
    const contents = [...this.parentElement.children];
    contents.forEach(content => {
        content.classList.remove("operations__content--active");
        if (content.classList.contains(`operations__content--${clicked.getAttribute("data-tab")}`))
